patches-own [ heat ]
turtles-own[new-heading turtle-heat]
extensions [vid]
...
to setup
  ...
  [ set size Turtle-size
    ...
    hide-turtle
    set turtle-heat initial-turtle-heat
  ]
  recolor-patches
  reset-ticks
end
...
to go
  if(auto = false)[
    ask turtles[
      ...
      set turtle-heat turtle-heat + heat-increase
  ...
  if(auto = true)[
    ask turtles[
      ...
      set turtle-heat turtle-heat + heat-increase
      set heat turtle-heat
      if (ticks mod tic-head) = 0[
        set new-heading heading + headchanger
      ]
      if (ticks mod tac-heat) = 0[
        set turtle-heat turtle-heat + heatchanger
      ]
    ]
  ]
...
to recolor-patches  ;; color patches according to heat
  ask patches [ set pcolor precision heat 1 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
626
...
247
Change!
set new-heading heading-choice
NIL
1
T
TURTLE
...
225
Save
vid:save-recording word name \".mp4\"
NIL
1
T
OBSERVER
...
The aim of this project is an visual artistic exhibition to show how a kaothic image could be explained clearly with the rules that run the turtles.

## HOW IT WORKS

**Auto: ON**

There is a **population** of turtles that has an **initial-turtle-heat**
